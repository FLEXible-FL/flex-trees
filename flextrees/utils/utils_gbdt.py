...
# Next package not needed as it is availabe in this file.
"""
try:
...
class LSHash(object):
    ...
    def index(self, input_point, extra_data=None):
        ...
        for i, table in enumerate(self.hash_tables):
            table.append_val(self.hash(self.uniform_planes[i], input_point),
                            value)
    ...
    def query(self, query_point, num_results=None, distance_func=None):
        ...
        if distance_func == "hamming":
            ...
            for i, table in enumerate(self.hash_tables):
                binary_hash = self.hash(self.uniform_planes[i], query_point)
